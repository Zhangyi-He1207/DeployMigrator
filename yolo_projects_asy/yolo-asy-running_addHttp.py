...
HTTP_SERVER_PID = 1136029  # HTTP服务端的固定PID
class ResourceMonitor(threading.Thread):
    def __init__(self):
        super().__init__(daemon=True)
        self.process = psutil.Process(os.getpid())
        self.http_process = psutil.Process(HTTP_SERVER_PID)
        self.cpu_percent_samples = []
        self.mem_samples = []
        self.http_cpu_samples = []
        self.http_mem_samples = []
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                http_cpu = self.http_process.cpu_percent(interval=0)
                http_mem = self.http_process.memory_info().rss / (1024 * 1024)  # MB
                self.http_cpu_samples.append(http_cpu)
                self.http_mem_samples.append(http_mem)
    ...
    def stop(self):
        ...
        http_stats = {
            "avg_cpu": sum(self.http_cpu_samples) / len(self.http_cpu_samples) if self.http_cpu_samples else 0,
            "max_cpu": max(self.http_cpu_samples) if self.http_cpu_samples else 0,
            "avg_mem": sum(self.http_mem_samples) / len(self.http_mem_samples) if self.http_mem_samples else 0,
            "max_mem": max(self.http_mem_samples) if self.http_mem_samples else 0,
        }
...
HTTP_UPDATE_URL = os.getenv("HTTP_UPDATE_URL", "http://localhost:8919/update_state")
def process_images():
    """异步处理图片的主循环"""
            ...
            with state_lock:
                ...
                try:
                    ...
                    response = requests.post(
                        HTTP_UPDATE_URL,
                        json=payload,
                        timeout=0.1  # 设置短超时避免阻塞
                    )
                    if response.status_code != 200:
                        logger.warning(f"状态更新失败: HTTP {response.status_code}")
                except Exception as e:
                    logger.error(f"HTTP状态更新异常: {str(e)}")
    ...
    # 当前应用资源统计
    logger.info(f"当前应用资源使用:")
    logger.info(f" - 平均CPU使用率: {stats['main']['avg_cpu']:.2f}%")
    logger.info(f" - 峰值CPU使用率: {stats['main']['max_cpu']:.2f}%")
    logger.info(f" - 平均内存使用: {stats['main']['avg_mem']:.2f} MB")
    logger.info(f" - 峰值内存使用: {stats['main']['max_mem']:.2f} MB")

    # HTTP服务端资源统计
    logger.info(f"HTTP服务端资源使用:")
    logger.info(f" - 平均CPU使用率: {stats['http']['avg_cpu']:.2f}%")
    logger.info(f" - 峰值CPU使用率: {stats['http']['max_cpu']:.2f}%")
    logger.info(f" - 平均内存使用: {stats['http']['avg_mem']:.2f} MB")
    logger.info(f" - 峰值内存使用: {stats['http']['max_mem']:.2f} MB")

    # 总资源消耗
    total_avg_cpu = stats['main']['avg_cpu'] + stats['http']['avg_cpu']
    total_max_cpu = stats['main']['max_cpu'] + stats['http']['max_cpu']
    logger.info(f"总资源消耗:")
    logger.info(f" - 平均CPU使用率: {total_avg_cpu:.2f}%")
    logger.info(f" - 峰值CPU使用率: {total_max_cpu:.2f}%")
